...
{
    public static readonly ConcurrentDictionary<string, UserSessionRecord> Sessions = new();
}

public class UserSessionStoreDictionary : IUserSessionStore
{
    public Task<IEnumerable<UserSessionRecord>> GetUserSessionsAsync(CancellationToken cancellationToken)
    {
        return Task.FromResult(UserSessionStoreDictionaryCache.Sessions.Values.AsEnumerable());
    }

    public Task<IEnumerable<UserSessionRecord>> GetUserSessionsByGroupAsync(string @group, CancellationToken cancellationToken)
    {
        return Task.FromResult(UserSessionStoreDictionaryCache.Sessions.Values.Where(x => x.Group == group));
    }

    public Task<IEnumerable<UserSessionRecord>> GetUserSessionsByFilterAsync(Expression<Func<UserSessionRecord, bool>> filter, CancellationToken cancellationToken)
    {
        return Task.FromResult(UserSessionStoreDictionaryCache.Sessions.Values.Where(filter.Compile().Invoke));
    }

    public Task AddUserSessionAsync(UserSessionRecord record, CancellationToken cancellationToken)
    {
    ...
    public Task RemoveAsync(Expression<Func<UserSessionRecord, bool>> filter, CancellationToken cancellationToken)
    {
